...
class MMGRudderForceModel : public ForceModel
{
    public:
        struct Yaml : MMGPropellerForceModel::Yaml
        {
            ...
            std::vector<double> gammaR;//!< Flow straightening coefficient (first value for negative $\beta_R$,second value for positive $\beta_R$)
            double epsilon;//!<Ratio of wake fraction at propeller and rudder positions
            double kappaMmg;//!< An experimental constant for expressing the fluid velocity at the rudder location
        ...
        struct RudderModel
        {
            ...
            /**  \brief Calculates the norm of the gradient force acting on the rudder
              ...
              */
            double get_Fn(const double rho, const double area, const double speed, const double rudder_inflow_angle) const;
...
const double rudder_angle, //!< Rudder angle (in radian): positive if rudder on port side
    ...
    const double rudder_angle, //!< Rudder angle (in radian): positive if rudder on port side
    const ssc::kinematics::Point Vs,           //!< Norm of the speed of the ship relative to the fluid (in m/s)
    const double area          //!< Rudder area (in or outside wake) in m^2
           ...
           double get_vr(const double u, const double v,const double r) const;
            ...
            private:
                ...
                double m_xH;//!< Longitudinal coordinate of acting point of the additional lateral force (in m)
                double m_lR;//!<Effective longitudinal coordinate of rudder position in formula of $\beta_R$ (in m)
                double m_tR;//!< Steering resistance deduction factor
                double m_aH;//!< Rudder force increase factor
                std::vector<double> m_gammaR;//!< Flow straightening coefficient (first value for negative $\beta_R$,second value for positive $\beta_R$)
                double m_epsilon;//!<Ratio of wake fraction at propeller and rudder positions
                double m_kappaMmg;//!< An experimental constant for expressing the fluid velocity at the rudder location
    ...
    private:
        MMGPropellerForceModel propulsionModel;//!< Propeller model
